...
export default function AuthForm<V extends FieldValues>({ title, description, fields, form, onSubmit, onError, isLoading, image, buttonText, redirectText, redirectButton, redirectLink, reverseGrid = false }: AuthFormProps<V>) {
                    ...
                    <form className="grid gap-4" onSubmit={form.handleSubmit(onSubmit, onError)}>
                        {fields.map((field) => (
                            <div className="grid gap-2" key={field.id}>
                                ...
                                <Input id={field.id} type={field.type} placeholder={field.placeholder} {...form.register(field.name)} />
                            </div>
                        ))}
